    ...
    <div className="mx-auto max-w-xl px-4 py-6">
      ...
      <form onSubmit={onSubmit} className="space-y-3 rounded-2xl p-4">
        <div>
          ...
          <input value={bankName} onChange={(e) => setBankName(e.target.value)} className="w-full rounded-lg border px-3 py-2" />
        </div>
        <div>
          <label className="mb-1 block text-sm">Account Number</label>
          <input value={account} onChange={(e) => setAccount(e.target.value)} className="w-full rounded-lg border px-3 py-2" />
        </div>
        <div>
          <label className="mb-1 block text-sm">IFSC Code</label>
          <input value={ifsc} onChange={(e) => setIfsc(e.target.value)} className="w-full rounded-lg border px-3 py-2" />
        </div>
        <div>
        ...
        <button type="submit" className="rounded-lg bg-black px-4 py-2 text-white">Withdraw</button>
      </form>
    </div>
  );
};
